...
public class Sotuken1 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			PreparedStatement pstmt1 = con.prepareStatement("select user_name from USERX join PROMEN using(user_id) where user_id = ?");
			pstmt1.setString(1, "1");
			ResultSet rs1 = pstmt1.executeQuery();
			rs1.next();
			request.getSession().setAttribute("user_name", rs1.getString("user_name"));
			
			PreparedStatement pstmt2 = con.prepareStatement("select pro_id,pro_name,leader_f from PRO join PROMEN using(pro_id) where user_id = ?");
